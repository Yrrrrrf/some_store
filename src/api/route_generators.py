...
def schema_dt_routes(
    ...
    def get_tables(): return [model.__tablename__ for model in all_models.values()]

    def create_column_route(model):
        @router.get(f"/{model.__tablename__}/columns", response_model=List[str], tags=["Columns"])
        def get_columns(): return [c.name for c in model.__table__.columns]
        return get_columns

    def get_all_resources_route(model, db_dependency):
        @router.get(f"/{model[0].__tablename__}/all", tags=["All"], response_model=List[model[1]])
        def get_all(
